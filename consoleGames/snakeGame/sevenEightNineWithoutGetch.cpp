...
void moveBackTwoMapsSoTheNextMapWillBePreviousSaveRelativeToScreen(vector<vector<tile> > &, playerStruct &);
...
int main() {
	...
	//init map once to populate the map vector
	initializeMap(game, vectorOfMaps, player);
	//After first map init call the coinSpawner function so we can create our first coin 
	coinSpawner(game, vectorOfMaps);
	//call init map a final time to update initial map with randomly placed first coin in game
	initializeMap(game, vectorOfMaps, player);
...
void initializeMap(gameValues game, vector<vector<tile> > &vectorOfMaps, playerStruct &player) {
	...
	if (player.loadLastSave && vectorOfMaps.size() > 1) {
		...
		moveBackTwoMapsSoTheNextMapWillBePreviousSaveRelativeToScreen(vectorOfMaps, player);
...
void moveBackTwoMapsSoTheNextMapWillBePreviousSaveRelativeToScreen(vector<vector<tile> > &vectorOfMaps, playerStruct &player){
